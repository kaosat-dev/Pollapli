...
class DeviceSqliteDaoTests(unittest.TestCase):    
    ...
    def setUp(self):
       ...
       # self._dbpool = adbapi.ConnectionPool("sqlite3",'pollapli.db',check_same_thread=False,cp_noisy=True,cp_max=1,cp_min=1)
...
#        self.assertEquals(obs,exp)
    ...
    def _insert_multiple_mockdevices(self):
        yield self._deviceSqliteDao._createTable()
        yield self._dbpool.runQuery('''INSERT into devices VALUES(null,"TestDeviceOne","A test description","inactive")''')
        yield self._dbpool.runQuery('''INSERT into devices VALUES(null,"TestDeviceTwo","Another test description","active")''')
