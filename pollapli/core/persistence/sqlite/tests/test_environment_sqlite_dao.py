...
class EnvironmentSqliteDaoTests(unittest.TestCase):
    ...
    def test_save_environment_new(self):
        input = Environment(name="TestEnvironment",description="A description")
    ...
    def test_load_environmentbyid(self):  
        input = Environment(name="TestEnvironment",description="A test description",status="frozen")
        yield self._environmentSqliteDao.save_environment(input)
        exp = input
        obs =  yield self._environmentSqliteDao.load_environment(id = 1)
        self.assertEquals(obs,exp)
    ...
    def test_load_all_environments(self):
        input = [Environment(name="TestEnvironmentOne",description="A test description",status="frozen"),Environment2(name="TestEnvironmentTwo",description="Another test description",status="live")]
